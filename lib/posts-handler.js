...
function handle(req, res) {
  ...
  switch (req.method) {
    case 'GET':
      ...
      if (trackingPassW === 123) {
        const deciTrackingId = cookies.get(trackingIdKey);
        if (isValidTrackingId(deciTrackingId, req.user)) {
        ...
        Post.findAll({ order: [['id', 'DESC']] }).then((posts) => {
          posts.forEach((post) => {
            ...
            post.content = cipher.update(post.content, 'utf8', 'hex');
            post.content += cipher.final('hex');
            //post.content = post.content.replace(/\+/g, ' ');
            post.formattedCreatedAt = moment(post.createdAt).tz('Asia/Tokyo').format('YYYY年MM月DD日 HH時mm分ss秒');
          });
          res.end(pug.renderFile('./views/posts.pug', {
...
function handleDecipher(req, res) {
  switch (req.method) {
    case 'POST':
    let body = [];
      req.on('data', (chunk) => {
        ...
        body = Buffer.concat(body).toString();
        const decoded = decodeURIComponent(body);
        const password = decoded.split('password=')[1];
        const trackingPass = 'hogehoge';
        if (password === trackingPass){
          trackingPassW = 123
        }
          handleRedirectPosts(req, res);
      });
