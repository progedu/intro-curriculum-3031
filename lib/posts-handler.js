// 現在の実装では、元々のトラッキング ID が推測可能なランダム関数を利用して作られています。
// Node.js の Math.random 関数は、アルゴリズムとして推測が行いやすいランダム値です。
// これを推測されにくい安全なランダム値のものに変更してください。
'use strict';
...
function addTrackingCookie(cookies, userName) {
  ...
  if (isValidTrackingId(requestedTrackingId, userName)) {
    ...
    // const originalId = Math.floor(Math.random() * Number.MAX_SAFE_INTEGER);// 削除

    //以上の値を用いることで、推測されにくい 8 バイトの整数値を得ることができます。
    // ただしJavaScriptの整数値の最大値は、2の53乗 - 1 であり、保持できる精度にも限りがあるためここで得られる数値は
    // 4738441943151179000のように下三桁から四桁が000や0000という値に丸められます。
