...
function addTrackingCookie(cookies, userName) {
  ...
  if (isValidTrackingId(requestedTrackingId, userName)) {
    ...
    //練習問題 originalIDの複雑化。cryptモジュールのrandomBytes関数で8バイトの整数を生成、toString 関数を文字列hexを引数として実行？、全体を16進数の整数に変換、最後にそれを整数に変換
...
//定数secretKeyでに長い文字列をあらかじめ代入
//この文字列はREPLでrequire('crypto').randomBytes(256).toString('hex');を実行して作られたもの
const secretKey =
...
function createValidHash(originalId, userName) {
  ...
  sha1sum.update(originalId + userName + secretKey);//secretKeyを付加した後にハッシュ化。練習問題でoriginalIDも修正
